...
(function(window, undefined) {
	...
	pro.__init = function(obj) {
		...
		if (obj.index == 1) {
			this.__preBtn.href = 'javascrpt:;';
		} else {
			this.__preBtn.href = obj.lianjie + (obj.index - 1);
		}
		...
		this.__nextBtn.href = obj.lianjie + (+1);
		if (obj.index == obj.total) {
			this.__nextBtn.href = 'javascrpt:;';
		} else {
			this.__nextBtn.href = obj.lianjie + (obj.index + 1);
		}
			...
			if (this.__index < 5) {
				for (var i = 0; i < 6; i++) {
					...
					if (this.__index == (i+1)) {
						...
						this.__numbers[i].href += (i + 1);
					}

					// console.log(this.__numbers[i].href);
				}
				this.__numbers[6].innerText = this.__total;
				this.__numbers[6].href += this.__total;
				...
				for (var i = 1; i < 7; i++) {
					...
					if ((this.__total + i - 6) == this.__index) {
						this.__numbers[i].href = "javascript:void(0)";
					} else {
						this.__numbers[i].href += (this.__total + i - 6);
					}
				}
				this.__numbers[0].innerText = '1';
				this.__numbers[0].href += 1;
				...
				for (var i = 1; i < 6; i++) {
					...
					if ((this.__index - 3 + i) == this.__index) {
						this.__numbers[i].href = "javascript:void(0)";
					} else {
						this.__numbers[i].href += (this.__index - 3 + i);
					}
					if (i == 3)
